...
// To test on windows
//var file_linking_method = utils.copy;
...
function download(url, options, callback) {
    if (env == 'development' && Object.keys(downloads).length > 1) {
        download_log_interval = setInterval(function() {
            ...
            if (download_last_count != in_use.length) {
            ...
            req.pipe(fs.createWriteStream(dl)).on('error', function(err) {
                ...
                if (req.response && req.response.statusCode >= 400) {
                    ...
                    fs.rename(dl, options.filepath, function(err) {
                        ...
                        // extension.
                        fs.writeFile(metapath(options.filepath), JSON.stringify(req.req.res.headers), 'utf-8', function(err) {
...
function localize(url, options, callback) {
    path.exists(options.filepath, function(exists) {
        if (exists) {
            callback(null, options.filepath);
        } else {
            var dir_path = path.dirname(options.filepath);
            utils.mkdirP(dir_path, 0755, function(err) {
...
function readExtension(file, cb) {
    fs.readFile(metapath(file), 'utf-8', function(err, data) {
        ...
        try {
            cb(null, guessExtension(JSON.parse(data)));
        } catch (e) {
            cb(e);
        }
    });
...
function resolve(options, callback) {
    ...
    Step(function setup() {
        ...
        resolved.Layer.forEach(function(l, index) {
            ...
            var linkpath = function(to, from, cb) {
                to = path.relative(cache, to).slice(0, 2) !== '..' ? path.relative(path.dirname(from), to) : to;
                if (env == 'development') console.log("[millstone] linking from '" + from + "' to '"  + to + "'");
                file_linking_method(to, from, cb);
            }
                    ...
                    case '.zip':
                        ...
                        path.exists(l.Datasource.file, function(exists) {
                            ...
                            unzip(file, function(err, file) {
                                ...
                                utils.processSHP(file, l.Datasource.file, linkpath, cb);
                            });
                        });
                        break;
                    // Symlink directories
                ...
                Step(function() {
                    ...
                    for (var i = 0; i < dbs.length; i++) (function(next) {
                        ...
                        var symlink = function(filepath, cb) {
                            ...
                            var to = path.relative(cache, filepath).slice(0, 2) !== '..'
                                ? path.relative(path.basename(filename), filepath)
                                : filepath;
                            file_linking_method(to, filename, cb);
                        };
