...
function checkTTL(cache, file, callback) {
    var uri = url.parse(file),
        filepath = path.join(cache, cachepath(file));

    var callback = _.isFunction(callback) ? callback : function(){};

    if (!uri.protocol) return callback(null, filepath);

    fs.stat(filepath, function(err, stats) {
        if (err && err.code != 'ENOENT') return callback(err);


        // the local file doesn't exist. just download it.
        if (err && err.code == 'ENOENT') {
            download(file, filepath, callback);
            return;
        }

        var lastDownloaded = new Date(stats.mtime);
        // Do a HEAD request against the remote file,
        // to determine wether it has been changed.
        var opts = { url: uri, method: 'HEAD' };
        ...
        request(opts, function(err, resp) {
            ...
            // only download if the server's copy is newer than ours.
            if (lastModified.getTime() > lastDownloaded.getTime()) {
                download(file, filepath, callback);
                return;
            }

            // don't replace the file, just pass through.
...
function resolve(options, callback) {
    ...
    Step(function setup() {
        ...
        resolved.Layer.forEach(function(l, index) {
            ...
            l.Datasource.ttl && checkTTL(cache, l.Datasource.file);
