...
// Working system will give ENOENT no such file or directory
try {
    ...
    if (e.code === 'ENOSYS') {
        ...
        if (env == 'development') {
            console.log("[millstone] **NOTICE** detected a user that does not have the privilege to create symlinks so falling back to copying data. To avoid this please ask your Administrator for SeCreateSymbolicLinkPrivilege rights (http://msdn.microsoft.com/en-us/library/bb530716%28VS.85%29.aspx) or run as Administrator");
        }
...
function guessExtension(headers) {
    if (headers['path']) {
        ...
        if (ext && (valid_marker_extensions[ext] || valid_ds_extensions[ext])) {
            return ext;
        }
    }
...
function readExtension(file, cb) {
    fs.readFile(metapath(file), 'utf-8', function(err, data) {
        ...
        try {
            ...
            if (env == 'development') console.log("[millstone] detected extension of '" + ext + "' for '" + file + "'");
            return cb(null, ext);
        } catch (e) {
            return cb(e);
        }
    ...
    Step(function setup() {
        ...
        resolved.Layer.forEach(function(l, index) {
                    ...
                    case '.zip':
                        if (nosymlink) {
                            unzip(file, function(err, file) {
                                ...
                                l.Datasource.file = file.replace('.zip','.shp');
                                return cb();
                            });
                        } else {
                            l.Datasource.file =
                            ...
                            exists(l.Datasource.file, function(exists) {
                                ...
                                unzip(file, function(err, file) {
                                    if (err) return cb(err);
                                    return utils.processSHP(file, l.Datasource.file, file_linking_method, {cache:cache}, cb);
                                });
                            });
                        }
                        break;
            ...
            // Absolute path.
                ...
                exists(pathname, function(exists) {
                    if (!exists && nosymlink) {
                    ...
                    Step(function() {
                        ...
                        case '.vrt':
                            ...
                            try {
                                ...
                                if (env == 'development') console.log('failed to open vrt file: ' + err.message);
                            }
                            break;
                        case '.csv':
                        case '.tsv':
