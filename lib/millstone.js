            ...
            if (d.type == 'sqlite' && d.table) {
                ...
                Step(function() {
                    ...
                    for (i = 0; i < dbs.length; i++) (function(next) {
                        ...
                        var file = dbs[i].split('@').shift();
                        if (file[0] != '/') {
                            file = path.resolve(path.join(base, uri.pathname));
                        }

                        db.run("ATTACH DATABASE '" + dbs[i].split('@').pop() + "' AS " + file, function(err) {
                            if (err) throw err;
                            next();
                        });
                    })(group());
                    ...
                    for (var i = 0; i < parts.length; i++) {
                        if (i % 2 == 0) (function(next) {
                            var table = parts[i].split(/ON/i).shift().split('.').pop().replace(/^\s*|\s*$/g, '');
                            db.all('PRAGMA table_info(' + table + ')', function(err, result) {
                                if (err) throw err;
                                for (i = 0; i < result.length; i++) {
                                    if (result[i].pk) {
                                        d.key_field = result[i].name;
                                    }
                                }
                                next();
                            });
