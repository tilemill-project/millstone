...
function isValidExt(ext) {
    return (ext && (ext == '.zip' || valid_marker_extensions[ext] || valid_ds_extensions[ext]));
}
...
function unzip(file, callback) {
    ...
    zf.names.forEach(function(name) {
        ...
        if (!path.extname(name) || name[0] === '.') {
            remaining--;
            if (!remaining) callback(null, ds_file);
        }
                    ...
                    case '.zip':
                        if (nosymlink) {
                            ...
                            unzip(file, function(err, file_found) {
                                ...
                                if (path.extname(file_found) == '.shp') {
                                    return utils.processSHP(file_found,
                                                            l.Datasource.file,
                                                            file_linking_method,
                                                            {cache:cache}, cb);
                                } else {
