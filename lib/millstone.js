    ...
    if (downloads[url]) {
        ...
        pool.acquire(function(obj) {
            (new get(url)).toDisk(dl, function(err, file, response, g) {
                var guessed_ext = g.guessResponseExtension(response);
                if (guessed_ext && extToMapnik[guessed_ext]) {
                    l.Datasource.type = extToMapnik[guessed_ext];
                }
                if (err) {
...
function resolve(options, callback) {
    ...
    Step(function setup() {
        ...
        resolved.Layer.forEach(function(l, index) {
            ...
            if (l.srs) {
            ...
            else if (d.type == 'shape' || ext == 'json' || ext == 'geojson') {
                // special handling for formats node-srs can handle:
                // shapefile's .prj and geojson files
                var next = group();
                var prj = path.join(
                    path.dirname(d.file),
                ...
                fs.readFile(prj, 'utf8', function(err, data) {
                    if (err && err.code === 'ENOENT') {
                        return next(new Error('No projection found for layer "' +
                            name + '" at ' + base));
                    } else if (err) {
                        return next(err);
                    }
                    ...
                    try {
                        ...
                        if (!l.srs)
                            l.srs = srs.parse('ESRI::'+data).proj4;
                    } catch (e) {
                        next(e);
                    }
                    ...
                    next(l.srs ?
                        null :
                        new Error('No projection found for layer "' +
                            name + '" at ' + base));
                });            
            } else {
                // all other formats, we need to request the user 
                // manually specify the SRS
                var next = group();
                next(new Error('No SRS is known for this layer type, please manually supply a projection for this layer: "' + name + '" at ' + d.file));
            }
