    ...
    if (env == 'development' && Object.keys(downloads).length > 1) {
        download_log_interval = setInterval(function() {
            ...
            if (in_use.length > 0 && (download_last_count != in_use.length)) {
                console.error("[millstone] currently downloading " + in_use.length + " files: " + _(in_use).map(function(f) { return path.basename(f); }));
            }
...
function add_item_to_metafile(metafile,item,callback) {
    if (exists(metafile)) {
...
function unzip(file, callback) {
    ...
    try {
        ...
        if (meta['unzipped_file'] & existsSync(meta['unzipped_file'])) {
            return callback(null,meta['unzipped_file']);
        }
    }
    catch (err) {
        if (env == 'development') console.error('[millstone] ' + err.message);
    }
    var zf;
    try {
