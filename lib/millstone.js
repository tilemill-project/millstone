        ...
        resolved.Layer.forEach(function(l, index) {
            ...
            var symlink = function(err, file) {
                ...
                case '.zip':
                    ...
                    path.exists(l.Datasource.file, function(exists) {
                        ...
                        unzip(file, function(err, file) {
                            if (err) return next(err);
                            utils.forcelink(path.dirname(file), path.dirname(l.Datasource.file), next);
                        });
                    });
                    break;
                // Symlink directories
                ...
                default:
                    ...
                    utils.forcelink(file, l.Datasource.file, next);
                    break;
                }
            };
            ...
            if (d.type == 'sqlite' && d.table) {
                ...
                Step(function() {
                    ...
                    for (i = 0; i < dbs.length; i++) (function(next) {
                        ...
                        db.run("ATTACH DATABASE '" + file + "' AS " + dbs[i].split('@').shift(), function(err) {
                            if (err) throw err;
                            next();
                        });
                    })(group());
                    ...
                    db.all("SELECT name FROM sqlite_master WHERE type='table' AND name='geometry_columns_auth'", function(err, result) {
                        if (err) throw err;
                    ...
                    _(parts).each(function(part, key) {
                        if (key % 2 === 0) (function(next) {
                            var table = part.split(/ON/i).shift().split('.').pop().replace(/^\s*|\s*$/g, '');
