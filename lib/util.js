...
function forcelink(src, dest, options, callback) {
    ...
    fs.lstat(dest, function(err, stat) {
        ...
        if (!stat.isSymbolicLink()) {
            if (env == 'development') console.log("[millstone] skipping re-linking '" + src + "' because '"  + dest + " is already an existing file");
            return callback();
        }
...
function copy(src, dest, options, callback) {
    ...
    fs.lstat(dest, function(err, dest_stat) {
        ...
        if (dest_stat.isSymbolicLink()) {
            if (env == 'development') console.log("[millstone] skipping copying '" + src + "' because '"  + dest + " is an existing symlink");
            return callback();
        }
