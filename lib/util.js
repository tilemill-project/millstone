...
// path.relative for node versions without one (node 0.4)
if ( ! path.relative ) path.relative = function(from, to) {
  var cf = path.dirname(from).split('/');
  var ct = path.dirname(to).split('/');

  // Find number of common dirs from the root
  var cd; 
  for (cd=1; cd<Math.min(cf.length, ct.length); ++cd) {
    if ( ct[cd] != cf[cd] ) break;
  }
  cd--;

  var newpath = [];

  // how many dirs to go back starting at "from"
  var backsteps = cf.length - cd;
  while (backsteps--) newpath.push('..');

  // add leftover dirs in "to"
  for (var i=cd+1; i<ct.length; ++i) newpath.push(ct[i]);

  // add filename
  newpath.push(path.basename(to));
...
function copyStream(src,dest,callback) {
    ...
    src_stream.on('end', function() {
        if (env == 'development') console.log("[millstone] finished copying '" + src + "' to '"  + dest + "'");
...
function copy(src, dest, options, callback) {
    ...
    fs.lstat(dest, function(err, dest_stat) {
        ...
        if (err && err.code === 'ENOENT') {
            if (env == 'development') console.log("[millstone] attempting to copy '" + src + "' to '"  + dest + "'");
        ...
        fs.lstat(src, function(err,src_stat) {
            ...
            if ((src_stat.size != dest_stat.size && src_stat.mtime < dest_stat.mtime) || src_stat.mtime > dest_stat.mtime) {
                if (env == 'development') console.log("[millstone] attempting to re-copy '" + src + "' to '"  + dest + "'");
                return copyStream(src,dest,callback);
            } else {
                return callback();
            }
